...
- 이 애플리케이션은 [http://thdtjsdn.com](http://thdtjsdn.com)의 다양한 실행형 서비스 모음
- 아래에서 제공되는 주요 기능들을 확인하고, 각 버튼을 통해 해당 서비스에 접근 가능
- 실행 파일이 관리자 권한이지만 악성코드류는 존재 하지 않음(감방 갈 생각 없음)
- 유료인증서 구매를 안해서 악성코드로 식별하는 케이스 존재함
- 윈도우 디펜더 또는 보안 프로그램 설정에서 해당 폴더를 제외하고 사용하세요
